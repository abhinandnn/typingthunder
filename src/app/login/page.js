...
<div className='text-[0.9rem] mob:text-[1.15rem] sm:hidden cursor-pointer text-white'>Test</div>
    ...
    <label className={`absolute pointer-events-none peer-focus:top-2 peer-focus:text-[0.875rem] ${error?'text-err':'peer-focus:text-dgr'}  ${!inputValue? 'top-[31.2%]': `top-2  text-dgr text-[0.875rem]`} left-[1rem]`} for="user">Enter username or email</label>
...
<div className='text-[1rem] gap-2 flex flex-row items-center '>
  <Image className='hover:cursor-pointer' onClick={()=>setCheck(!isCheck)} src={isCheck?checkbox:checkbox1}/>
Remember me</div>
  </div>
  <button className='bg-white text-black text-[1.375rem] w-[80vw] sm:w-[31.7rem] h-[4rem] flex flex-row items-center justify-center px-6 pt-3 pb-[0.625rem] rounded-[1.25rem]'>
{loading?<Image src={loading1} className='animate-spin'/>:"Continue"
}</button>
<button type='button' onClick={handleGoogle} className='bg-transparent flex flex-row justify-center items-center gap-4 border border-dgr text-[1.125rem] w-[80vw] sm:w-[31.7rem] h-[4rem] px-[1rem] sm:px-[8.56rem] py-[1rem] rounded-[1.25rem]'>
...
<div className='mt-4 text-[#B3B3B3] sm:hidden text-[1.125rem] flex justify-center text-center'>Don't have an account? <span className='text-[#e6e6e6] underline'><Link href={'/signup'}>Sign up</Link></span></div>
