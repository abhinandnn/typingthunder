...
<div className='text-[#B3B3B3] hidden sm:inline text-[1.125rem]'>Already a user? <Link href={'/login'}><span className='text-[#e6e6e6] underline'>Log in</span></Link></div>
    ...
    <div className='flex flex-row items-center justify-center gap-[12.5rem]'>
    <div className='hidden xl:flex pt-[10rem] w-[30rem] flex-col gap-[1.5rem]'>
        <Image src={Otpil}/>
        </div>
        <div className='flex flex-col w-[80vw] sm:w-auto gap-3 mob:gap-6 text-[1.125rem] pt-[4.8rem] text-white'>
          ...
          <div className='flex flex-col relative pb-2 mob:pb-4'>
          <OTPInput
      value={otp}
      onChange={handleOtp}
      numInputs={6}
      ...
      inputType='tel'
      inputStyle={`mob:mr-4 mr-2 min-h-[2.5rem] min-w-[2.5rem] mob:min-h-[3.125rem] mob:min-w-[3.125rem] text-[1.125rem] mob:text-[1.375rem] bg-transparent text-white rounded-[0.8125rem] border ${(otp.length==6&&!error)||otp.length==0?'border-dgr':'border-[#FF7E7E]'} outline-none` } 
    />
    {error&&otp.length>0&&<span className='text-[1rem] text-err absolute l-0 bottom-[-12%]'>{error}</span>}
    </div>
    <ResendOtp email={em}/>
    <button onClick={handleSubmit} className='absolute bottom-[16%] mob:static bg-white active:bg-blue text-black text-[1.375rem] w-[80vw] sm:w-[31.7rem] h-[4rem] flex flex-row justify-center items-center px-6 pt-3 pb-[0.625rem] rounded-[1.25rem]'>
  {loading?<Image src={loading1} className='animate-spin'/>:"Verify"
}
    </button>
 </div>
    </div> 
<div className='mt-4 absolute bottom-[6%] mob:static text-[#B3B3B3] sm:hidden min-w-[80vw] text-[1.125rem] flex justify-center text-center'>Already have an account? <span className='text-[#e6e6e6] underline'><Link href={'/login'}>Log in</Link></span></div>
